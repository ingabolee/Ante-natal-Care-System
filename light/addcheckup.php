...
if(isset($_POST['submit'])){
    ...
    $checkup_mother_id = $_POST['checkup_mother_id'];

    $sql = "INSERT INTO checkup (checkup_appointment_date, checkup_description, checkup_mother_id) 
                VALUES ('$checkup_date', '$checkup_description', '$checkup_mother_id')";

    $result = mysqli_query($conn, $sql);
    if ($result){
        $sql = "SELECT * FROM conversation WHERE conversation_mother_id='$checkup_mother_id'";

        $result = mysqli_query($conn, $sql);
        if($result->num_rows < 1){
            $sql = "INSERT INTO conversation (conversation_doctor_id, conversation_mother_id) 
            VALUES ('$doctor_id', '$checkup_mother_id')";
                        ...
                        <div class="row clearfix">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" placeholder="checkup Date" name="checkup_date">
                                </div>
                            </div>
                            <br>
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label for="">Checkup Description</label>
                                    <input type="text" class="form-control" placeholder="checkup description" name="checkup_description">
                                </div>
                            </div>
                            <br>
                            
                            <div class="float-right">
                            <h6 class="mt-4">Mother</h6><br>
                                <div class="col-sm-6">
                                <?php   while($row = mysqli_fetch_assoc($rows)):?>
                                    <div class="radio inlineblock m-r-25">
                                        <input type="radio" id="<?php echo $row['mother_id']; ?>" name="checkup_mother_id" value="<?php echo $row['mother_id']; ?>">
                                        <label for="<?php echo $row['mother_id']; ?>"><?php echo $row['mother_first_name'];?></label>
                                    </div>
                                    <?php  endwhile; ?> 
                                </div>
                            </div>
                            ...
                            <div class="col-sm-12">
                                <button type="submit" name="submit" class="btn btn-primary btn-round">Add checkup</button>
